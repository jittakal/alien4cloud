...
public class SetupStepDefinitions {
    ...
    @And("^I upload without checking the result the local archive \"([^\"]*)\"$")
    public void I_upload_withoud_checking_the_result_the_local_archive(String archive) throws Throwable {
        Path archivePath = Context.LOCAL_TEST_DATA_PATH.resolve(archive);
        uploadWithoutChecking(archivePath);
    }
